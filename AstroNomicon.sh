...
[ ! -e "${TMPFILE}" ] && echo 'inactive=yes' > "${ETCFILE}" && exit 1

## There's solace in power
declare -a vars=( $(/bin/cat "${TMPFILE}" | cut -d'=' -f 1) )
echo -e "$HEADER\ndate_check=$(date +%Y-%m-%d)" > "${ETCFILE}"
(
  . "${TMPFILE}"

  for v in ${vars[@]}; do
    ( ! [[ ${!v} =~ [^0-9]+ ]] ) && echo "${v}=${!v}">>${ETCFILE} && continue
    echo "${v}=$(date --date "${!v}" +"$CONFIG_DATE_FORMAT")">>"${ETCFILE}"
  done
) # as-if
